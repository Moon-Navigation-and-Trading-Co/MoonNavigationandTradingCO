...
export default function CarouselAnimation({ freightTypes, title }: CarouselAnimationProps) {
    ...
    return (
        <div className={`flex flex-col lg:flex-row h-[700px] lg:h-[500px] w-full gap-4 my-16 px-2`}>
            <div className="w-full relative shadow-md overflow-hidden shadow-black h-[150px] lg:h-full lg:w-[160px] rounded-2xl flex items-center justify-start">
                ...
                <h1 className="text-center z-[105] text-3xl lg:text-xl font-semibold text-[#E2F1E7] w-full">{title}</h1>
            </div>
                    ...
                    >
                        <CardContent className="relative w-full h-full p-0">
                            ...
                            <div className="absolute w-full h-full bottom-0 left-0 px-2 py-4 text-white">
                                {expandedIndex !== index && <h2 className="text-lg flex text-center justify-start items-end w-full h-full lg:justify-center font-semibold">{freight.title}</h2>}
                                {expandedIndex === index && (
                                    <motion.div
                                        initial={{ opacity: 0, y: 20 }}
                                        animate={{ opacity: 1, y: 0 }}
                                        transition={{ delay: 0.3, type: 'linear' }}
                                        className="h-full w-full flex p-2 flex-col justify-between lg:justify-end gap-2"
                                    >
